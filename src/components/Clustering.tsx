...
export default function Clustering() {
  ...
  const processClustering = async () => {
    ...
    try {
      ...
      // Register students if not exists
        ...
        if (!existingUser) {
          ...
          await supabase
            ...
            .insert({
              ...
              email: `${result.NIM}@student.ac.id`, // Generate email from NIM
              role: 'mahasiswa',
              tingkat: result.TINGKAT,
              kelas: result.KELAS,
              level_user: 0 // Student level
            });
        }
      } catch (error) {
  ...
  const saveResults = async () => {
    ...
    try {
      ...
      await databaseAPI.saveClusteringResults(resultsWithUserIds, selectedBatch);
      setMessage({ type: 'success', text: 'Hasil clustering berhasil disimpan!' });
      ...
      {showSaveModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-96 max-w-md mx-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Simpan Hasil Clustering</h3>
